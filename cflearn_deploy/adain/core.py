...
from ..toolkit import quantile_normalize
...
class AdaINStylizer:
    ...
    def _get_stylized(
        ...
        stylized = self.onnx.run({"input": content, "style": style})[0]
        stylized = quantile_normalize(stylized, q=q)[0]
        stylized = stylized.transpose([1, 2, 0])
        return to_uint8(stylized)

    def run(self, img_bytes0: bytes, img_bytes1: bytes, q: float) -> np.ndarray:
        content = bytes_to_np(img_bytes0, mode="RGB")
        style = bytes_to_np(img_bytes1, mode="RGB")
        return self._get_stylized(content, style, q)
